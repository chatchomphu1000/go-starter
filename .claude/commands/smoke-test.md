...
### [4/7] Register duplicate → expect 409 Conflict
```bash
curl -s -w "\n%{http_code}" -X POST http://localhost:8080/api/v1/users/register \
  -H "Content-Type: application/json" \
  -d "{\"name\":\"Smoke Test\",\"email\":\"$TEST_EMAIL\",\"password\":\"$TEST_PASSWORD\"}"
...
```
PASS: 200, extract `access_token` → store as `TOKEN`

### [6/7] Login with wrong password → expect 401
```bash
curl -s -w "\n%{http_code}" -X POST http://localhost:8080/api/v1/auth/login \
  -H "Content-Type: application/json" \
...
### [7/7] Get own profile (authenticated)
```bash
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  [3] POST /api/v1/users/register   [ PASS | FAIL ] 201
  [4] POST /register (duplicate)    [ PASS | FAIL ] 409
  [5] POST /api/v1/auth/login       [ PASS | FAIL ] 200
  [6] POST /login (wrong pwd)       [ PASS | FAIL ] 401
  [7] GET  /api/v1/users/:id        [ PASS | FAIL ] 200

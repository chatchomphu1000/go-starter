...
func (s *userService) Register(ctx context.Context, in inbound.RegisterInput) (*domain.User, error) {
	...
	if exists {
		return nil, fmt.Errorf("userService.Register: %w", domain.ErrEmailAlreadyExists)
	}
...
func (s *userService) Login(ctx context.Context, in inbound.LoginInput) (*inbound.AuthToken, error) {
	if err := ctx.Err(); err != nil {
	...
	if !user.Active {
		return nil, fmt.Errorf("userService.Login: %w", domain.ErrUserInactive)
	}
...
func (s *userService) List(ctx context.Context, f inbound.ListFilter) ([]*domain.User, int64, error) {
	if err := ctx.Err(); err != nil {
...
func (s *userService) Update(ctx context.Context, id string, in inbound.UpdateInput) (*domain.User, error) {
	if err := ctx.Err(); err != nil {
	...
	if in.Name != nil {
		if err := user.Rename(*in.Name); err != nil {
			return nil, fmt.Errorf("userService.Update: %w", apperrors.BadRequest(err.Error(), err))
		}
	}
